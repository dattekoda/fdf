...
/*   Updated: 2025/08/05 15:18:38 by khanadat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	calc_color(t_line *line, t_point current, bool big)
{
	...
	start = line->s->color;
	goal = line->g->color;
	red = get_grad((start >> 16) & 0xFF, (goal >> 16) & 0xFF, p);
	green = get_grad((start >> 8) & 0xFF, (goal >> 8) & 0xFF, p);
	blue = get_grad(start & 0xFF, goal & 0xFF, p);
